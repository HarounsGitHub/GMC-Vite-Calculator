...
function handleOperation(op) {
    ...
    if (lastOperation && !wasLastOp && (!wasEqual || op == '=')) {
        ...
        if (lastOperation == "+") {
            ...
            //result = savedScreen / screen.innerHTML we should flip this on else as we did in substraction
            //Develop the previous line into two if & else (in case we use = to / more)
            if (wasEqual && op == '=') result = screen.innerHTML / savedScreen
            else result = savedScreen / screen.innerHTML
        }
...
// 16 FEBUARY 2023
//We are making an event that let us make input (not only by clicking) but also by keyboard
export function setupKeyboard() {
    //screen.addEventListener is useless because its HTML div doesn't have an input
    window.addEventListener('keyup', (event) => {
        // console.log('keyboard pressed')
        // console.log('event', event)
        // const pressedKey = event.key
        // const pressedKey = event.key++ '*' ? 'x' : event.key
        const pressedKey =
        ...
        // console.log('pressedKey0' + pressedKey)
        console.log('pressedKey ', event.key)
        console.log('event', event)

        if (pressedKey == 'Delete') handleFunc('reset')
        else if (pressedKey == 'Backspace') handleFunc('del')
    })
}